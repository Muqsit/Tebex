...
final class XuidBasedPlayerIndexer implements PlayerIndexer{
	...
	public function fromPlayer(Player $player) : string{
		$xuid = $player->getXuid();
		if($xuid === ""){
			throw new InvalidStateException("Cannot retrieve XUID of player: {$player->getName()}");
		}
		return $xuid;
	}
	...
	public function fromTebexDuePlayer(TebexDuePlayer $player) : string{
		...
		if($xuid === null){
			throw new InvalidStateException("Cannot retrieve XUID of Tebex due player: {$player->getName()}");
		}
		return $xuid;
	}
}